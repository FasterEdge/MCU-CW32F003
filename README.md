...
<h2>FasterEdge MCU - CW32F003</h2>
<h3>FasterEdge 框架的 CW32F003（Cortex-M0+） 平台实现（Keil MDK / PlatformIO 版）</h3>
</div>

### 一、简介

本项目是 **[FasterEdge](https://github.com/FasterEdge/FasterEdge)** 框架在 ****CW32F003（Cortex-M0+）****平台上的实现。CW32F003 为 32 位 Cortex-M0+ 内核（武汉芯源）：48MHz、32KB Flash、4KB RAM、无硬件 EEPROM，无网络、无操作系统，因此按 [MCU-C51](../MCU-C51) 的无网络精简思路裁剪能力子集，并保留 **寄存器 / GPIO / 芯片信息** 三个 MCU 专有模块。

- ✅ **keil/（Keil MDK，官方）** + **platformio_ide/（ARM GCC，PlatformIO）** 双版本
- ✅ 与主仓库**同名同命令**，云边协同对等编程
- ✅ HMAC-SHA256 纯 C 零依赖
- ✅ 配置/密钥持久化到片上 Flash 模拟 EEPROM（参考）
- ✅ platformio_ide 版为 **Cortex-M0+ 寄存器级实现**（RCC / GPIO / USART1 / SysTick）
...
| 能力 | 排除原因 |
|------|---------|
| MQTTAbility / NetMapData | CW32F003 无网络协议栈 |
...
```
MCU-CW32F003/
├── keil/                       # Keil MDK 版（uVision 工程）
│   ├── MDK-ARM/                # FasterEdge-MCU-CW32F003.uvproj（ARM Compiler）
...
└── platformio_ide/             # VS Code + PlatformIO IDE 工程（ARM GCC）
    ├── platformio.ini          # board = cw32f003（需自建 board 或本地编译）
    ├── include/                # fe.h / fe_ability.h / fe_data.h / fe_port.h / fe_hmac_sha256.h
    └── src/                    # 裸机 C + 寄存器级 fe_port（Cortex-M0+）
```

> 两套均为裸机 C 工具链：`keil/`（Keil MDK 官方编译器）与 `platformio_ide/`（ARM GCC），能力与命令完全一致。

### 五、使用说明

1. **keil 版**：用 Keil MDK 打开 `keil/MDK-ARM/FasterEdge-MCU-CW32F003.uvproj`（或按芯片新建 ARM 工程），SWD 烧录
2. **platformio_ide 版**：PlatformIO IDE 打开 `platformio_ide/`（CW32 无官方 PlatformIO board，可用本地 Makefile/CMake 或自建 board）
3. 在 `fe_port.c` 中按你的 CW32 型号调整 UART/GPIO 引脚（已附寄存器参考）
...
| 差异点 | ESP32/ESP8266 | CW32F003 |
|--------|--------------|---------------------|
| 架构 | Xtensa 32 位 | **ARM Cortex-M0+ 32 位** |
| RAM / Flash | KB~MB | **4KB SRAM / 32KB Flash** |
| 存储 | NVS / Flash | **片上 Flash 模拟 EEPROM（参考）** |
| 网络 | 有 | **无**（能力子集剔除网络项）|
| 寄存器 | 32 位内存映射 | **32 位外设空间 0x40000000+（RegAbility 宽度 32）** |

### 七、platformio_ide 版实现说明（ARM GCC）

`platformio_ide/` 为 **ARM GCC 版** 工程，`fe_port.c` 为 Cortex-M0+ 寄存器级参考实现（RCC/GPIO/USART/SysTick；GPIO 寄存器风格与 STM32F0 相近，可按 CW32 官方头文件微调地址）。

```bash
cd platformio_ide
pio run            # 编译（需自建 board 或 CMake）
pio run -t upload  # 烧录
...
除主仓库对应能力外，本仓库提供 3 个 **MCU 专有** 模块（寄存器 / GPIO / 芯片信息）。CW32F003 的寄存器为 **32 位 Cortex-M0+ 外设空间**（0x40000000+），GPIO 为引脚号 0-19：

| 模块 | 类型 | 命令 | 说明 |
|------|------|------|------|
| RegAbility | Ability | `read <addr>` / `write <addr>,<value>` / `bit_set <addr>,<bit>` / `bit_clear <addr>,<bit>` / `info` | ARM 内存映射寄存器（32 位，volatile 指针） |
| GpioAbility | Ability | `mode <pin>,<input\|output\|input_pullup>` / `write <pin>,<0\|1>` / `read <pin>` / `info` | GPIO 引脚（pin 0-19，寄存器级） |
| ChipData | Data | `info` | CW32F003 型号 / RAM / Flash / 频率 |

**示例：**

```
ability_RegAbility read 0x40015800      # 读 GPIO 外设
ability_RegAbility write 0x40015814,0x01 # 写 GPIO BSRR
