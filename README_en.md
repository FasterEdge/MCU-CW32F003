...
<h3>FasterEdge framework on CW32F003 (Cortex-M0+) (Keil MDK / PlatformIO editions)</h3>
</div>

### 1. Introduction

This repo implements the **[FasterEdge](https://github.com/FasterEdge/FasterEdge)** framework on the **CW32F003 (Cortex-M0+)**. The CW32F003 is a 32-bit Cortex-M0+ (Whxy): 48MHz, 32KB Flash, 4KB RAM, no HW EEPROM — no network, no OS. Following the [MCU-C51](../MCU-C51) no-network design, the capability set is trimmed and 3 **MCU-specific** modules (registers / GPIO / chip info) are kept.
...
| Capability | Reason |
|------------|--------|
| MQTTAbility / NetMapData | No network stack on CW32F003 |
...
└── platformio_ide/             # VS Code + PlatformIO IDE project (ARM GCC)
    ├── platformio.ini          # board = cw32f003 (self-built board or local build)
    ├── include/                # fe.h / fe_ability.h / fe_data.h / fe_port.h / fe_hmac_sha256.h
    └── src/                    # bare-metal C + register-level fe_port (Cortex-M0+)
```

> Both are bare-metal C toolchains: `keil/` (official Keil MDK) and `platformio_ide/` (ARM GCC), identical capabilities & commands.

### 5. Usage

1. **keil edition**: open `keil/MDK-ARM/FasterEdge-MCU-CW32F003.uvproj` in Keil MDK (or create an ARM project), SWD flash
2. **platformio_ide edition**: PlatformIO IDE open `platformio_ide/` (CW32 has no official PlatformIO board; use local Makefile/CMake or self-built board)
3. Adjust UART/GPIO pins in `fe_port.c` for your CW32 model (register reference included)
...
The `platformio_ide/` edition is the **ARM GCC** project; `fe_port.c` is a Cortex-M0+ register-level reference implementation (RCC/GPIO/USART/SysTick; GPIO registers are STM32F0-like — adjust addresses per the CW32 official header).

```bash
cd platformio_ide
pio run            # build (self-built board or CMake)
...
Beyond main-repo capabilities, 3 **MCU-specific** modules (registers / GPIO / chip info) are provided. The CW32F003 registers are **32-bit Cortex-M0+ peripheral space** (0x40000000+); GPIO uses pin numbers 0-19:
...
| ChipData | Data | `info` | CW32F003 model / RAM / Flash / freq |

**Examples:**

```
ability_RegAbility read 0x40015800      # read GPIO peripheral
ability_RegAbility write 0x40015814,0x01 # write GPIO BSRR
